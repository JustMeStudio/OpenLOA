...
async def create_content_using_hint(hint:str):
    ...
    prompt ="本次写作相关线索如下：\n" + hint + "\n" + "作为参考，以下是一些过往的完整作品：\n"+ relative_demos
    qprint(prompt) #debug
    content = request_LLM_api(prompt, system_prompt="你会根据用户提供的本次创作的线索，仿造过往写作风格完成本次写作，注意:\n1.你的篇幅长度不要与过往作品差异过大\n2.你的语言风格一定要和以往作品非常接近！\n3.创作内容中禁止出现以往作品里或本次创作线索中没提到过的内容！\n")
    if content:
        #保存到本地
        save_path = f"{globals.SUBMIT_PATH}/output.txt"
...
if  __name__ == "__main__":
    # result = record_content_to_database("./test.csv")
    result = create_content_using_hint("阿拉斯加雪橇犬")
    qprint(result)